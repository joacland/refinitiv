...
out_file = pl.Path.joinpath(out_path, "instrumentid_date_range.csv")
ret_file = pl.Path.joinpath(download_path, "tret.csv")
err_file = pl.Path.joinpath(raw_path, "no_data_instrumentid.csv")
...
if __name__ == "__main__":
    ...
    own_list = own_list.dropna(
        ...
        subset=[
            "InstrumentID",
            "QuoteID"
        ],  # Must be possible to trace Instrument and Quote to Organization
    )

    # Read already collected ID vars
    existing_id = own.read_csv_file(out_file)
    existing_id = existing_id["InstrumentID"]
    ...
    # Add known price close date (QuoteID-level data)
    ...
    ### Fix possible non-unique InstrumentID to date range min(SDate) -- max(EDate)
    first_last_dates["SDate"] = first_last_dates.groupby(
        "InstrumentID"
    ).SDate.transform("min")
    first_last_dates["EDate"] = first_last_dates.groupby(
    ...
    first_last_dates = (first_last_dates.merge(existing_id, on="InstrumentID", how="left", indicator=True)
                        .query('_merge == "left_only"')
                        .drop(labels="_merge", axis=1))

    ### Convert into a dictionary (and Instruments as a list)
    first_last_dates = first_last_dates[["InstrumentID", "SDate", "EDate"]]
    first_last_dates = first_last_dates.sort_values("InstrumentID")
    first_last_dates.drop_duplicates(subset=["InstrumentID"], inplace=True)
    ...
    own_list = (own_list.merge(existing_id, on="InstrumentID", how="left", indicator=True)
                        .query('_merge == "left_only"')
                        .drop(labels="_merge", axis=1))
    own_list.drop_duplicates(subset=["InstrumentID"], inplace=True)

    own_list = own_list["InstrumentID"]
    ...
    first_last_dates = first_last_dates.set_index("InstrumentID")
    date_dict = first_last_dates.to_dict(
        "index"
    )  # Dictionary w/ SDate and EDate per QuoteID

    # RETRIEVE DATA FROM EIKON
    print(f"No of 'InstrumentID' to retrieve data for: {str(len(own_list))}.")
    ...
    for qte in own_list:
        ...
        print(f" - InstrumentID {qte} for period {sdate} -- {edate}. #{counter}/{str(len(own_list))}")
        own_dict = {
            # "Period": period,
            "SDate": sdate,
            "Edate": edate,
        ...
        for rec_attempts in range(20):
            ...
            # All attempts failed
            print(
                f"All attempts have failed: Program aborted while running InstrumentID {qte}"
            )
            sys.exit()
